"""
URL configuration for twitterclone project.

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/5.1/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""
from django.contrib import admin
from django.urls import path, include
from django.shortcuts import redirect
from django.conf import settings # import settings.py
from django.conf.urls.static import static # for static
from django.contrib.auth.urls import views as auth_views

def redirect_to_twitter(request):
    return redirect('tweet_list')

urlpatterns = [
    path('admin/', admin.site.urls),

    # Redirect root to /twitter
    path('', redirect_to_twitter),

    path('twitter/', include('twitter.urls')), # Twitter app url

    path('accounts/', include('django.contrib.auth.urls')), # Login-Register
    

] + static(settings.MEDIA_URL, document_root = settings.MEDIA_ROOT)
